            ...
            <div className="space-y-12 animate-fade-in">
                ...
                <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                    <div className="space-y-2">
                        <span className="section-label">Institutional Management</span>
                        <h1 className="text-4xl font-black text-white tracking-tighter">Industry Supervisor</h1>
                        <p className="text-slate-500 font-medium leading-relaxed">Overseeing student placements and operational compliance.</p>
                    </div>
                    <div className="flex items-center space-x-3 bg-blue-600/10 text-blue-400 px-6 py-3 rounded-2xl border border-blue-600/20 shadow-inner">
                        <BarChart3 size={20} />
                        <span className="text-sm font-black uppercase tracking-widest">System Live</span>
                    </div>
                </div>
                ...
                <div className="grid lg:grid-cols-3 gap-10">
                    ...
                    <div className="lg:col-span-1 space-y-6">
                        <div className="glass-card p-10 flex flex-col items-center justify-center space-y-8 bg-gradient-to-b from-white/[0.05] to-transparent">
                            <div className="text-center w-full space-y-2">
                                <h3 className="text-xl font-black text-white tracking-tight">Clock-in Students</h3>
                                <p className="text-slate-500 text-sm font-medium">Scan QR to verify presence</p>
                            </div>

                            {!isScanning ? (
                                <div
                                    onClick={() => setIsScanning(true)}
                                    className="w-full aspect-square bg-white/[0.02] border-2 border-dashed border-white/10 rounded-[2.5rem] flex flex-col items-center justify-center cursor-pointer hover:bg-white/5 hover:border-blue-500/30 transition-all group"
                                >
                                    <div className="w-20 h-20 bg-blue-600/10 text-blue-400 rounded-3xl flex items-center justify-center mb-4 group-hover:scale-110 transition-transform shadow-inner">
                                        <Camera size={40} />
                                    </div>
                                    <span className="text-slate-500 font-black text-[10px] uppercase tracking-[0.2em] group-hover:text-blue-400">Activate Camera</span>
                                </div>
                            ) : (
                                <div id="reader" className="w-full overflow-hidden rounded-[2.5rem] border-4 border-blue-600/50 shadow-2xl" />
                            )}
                    ...
                    <div className="lg:col-span-2 space-y-10">
                        ...
                        <div className="space-y-6">
                            {[
                                { name: 'Alvin Mutie', week: 9, submissionDate: '24 Jan 2026', avatar: 'AM', school: 'School of Computing' },
                                { name: 'Sarah Wilson', week: 9, submissionDate: '25 Jan 2026', avatar: 'SW', school: 'Business School' },
                            ].map((sub, i) => (
                                <div key={i} className="glass-card p-10 flex flex-col md:flex-row md:items-center justify-between gap-8 group hover:border-blue-600/30 transition-all bg-gradient-to-r from-blue-600/5 to-transparent">
                                    <div className="flex items-center space-x-6">
                                        <div className="w-16 h-16 bg-blue-600/20 rounded-3xl flex items-center justify-center font-black text-blue-400 text-xl border border-blue-600/20">
                                            {sub.avatar}
                                        </div>
                                        <div className="space-y-1">
                                            <h4 className="text-xl font-black text-white tracking-tight">{sub.name}</h4>
                                            <p className="text-slate-500 text-xs font-bold uppercase tracking-widest">{sub.school} • Week {sub.week}</p>
                                            <div className="flex items-center space-x-2 pt-2 text-slate-600 text-[10px] font-black uppercase tracking-[0.2em]">
                                                <Clock size={12} />
                                    ...
                                    <div className="flex items-center space-x-4">
                                        <button className="bg-blue-600 text-white px-8 py-3.5 rounded-2xl text-xs font-black uppercase tracking-widest shadow-xl shadow-blue-900/40 hover:bg-blue-500 transition-all flex items-center space-x-3">
                                            <span>Full Review</span>
                                            <ArrowUpRight size={18} />
                                        </button>
                                        <button className="p-3.5 rounded-2xl bg-white/5 text-slate-400 border border-white/10 hover:bg-white/10 hover:text-white transition-all">
                                            <XCircle size={22} />
                                        </button>
                                    </div>
                                </div>
